    ...
    if (fin.is_open())
    {
        ...
        while(/*(getline(fin, linha)*/ fin >> nome)
        {
            ...
            switch (nome[0])
            {
                ...
                case 'K':
                    fin >> no[0] >> no[1] >> valor[0];
                    for(i = 0; i < (int)listaElementos.size(); i++)
                    {
...
void Circuito::resolveSistema()
{
    ...
    int i, j, k, aux;
    for (i = 0; i < (int)listaInc.size() - 1; i++)
        p[i] = i + 1;
    for (i = 0; i < (int)listaInc.size() - 2; i++)
    {
        for (j = i + 1; j < (int)listaInc.size() - 1; j++)
        ...
        for (j = i + 1; j < (int)listaInc.size() - 1; j++)
        {
            ...
            for (k = i; k < (int)listaInc.size() - 1; k++)
